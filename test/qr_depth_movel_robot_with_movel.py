# qr_depth_movel_robot_qr5cm_pose_debug.py
# Robot side
# 목표:
...
# - QR 5cm pose와 RealSense depth를 함께 확인하고 데드존 밖이면 반복 보정
...
def handle_visual_approach_once(mc, command):
    """
    QR 실제 크기 5cm 기반 반복 보정 방식입니다.

    방식:
    1. PC가 보낸 QR 중심 픽셀(cx, cy)과 4개 코너 pts를 받음
    2. QR 실제 크기 5cm + RealSense intrinsics로 solvePnP pose 계산
    3. RealSense depth 중심값과 solvePnP 거리값을 비교해 신뢰도 디버그 출력
    4. 목표 위치 [0, 0, FINAL_STANDOFF_M]와 현재 QR pose의 차이를 계산
    5. MoveL로 1회 이동
    ...
    print(
        f"[QR 5CM POSE] source={pose_source} depth_status={depth_status} "
    ...
    if center_in_deadzone and depth_in_deadzone:
        print(
            ...
            f"px=({pixel_ex:+.1f},{pixel_ey:+.1f}) poseZ={point_z_m*100:.1f}cm"
        )
        safe_stop(mc)
        approach_done = True
        return {
            ...
            "debug_reason": "QR 중심 오차와 pose 기반 거리 오차가 모두 데드존 안이라 정지했습니다.",
            "deadzone": {
    ...
    if correction_count >= MAX_CORRECTION_COUNT:
        ...
        print(
            ...
            f"poseZ={point_z_m * 100:.1f}cm count={correction_count}"
        )
        return {
            "ok": True,
    ...
    # QR 5cm pose로 얻은 카메라 좌표계 중심 위치를 직접 보정 벡터로 사용합니다.
    ...
    print(
        f"[STATE] need_correction_5cm_pose: center_in_deadzone={center_in_deadzone} "
        f"depth_in_deadzone={depth_in_deadzone} "
        f"px=({pixel_ex:+.1f},{pixel_ey:+.1f}) "
        f"pose_cm=({point_x_m*100:+.1f},{point_y_m*100:+.1f},{point_z_m*100:.1f}) "
        f"delta_cam_cm=({delta_cam_m[0]*100:+.1f},{delta_cam_m[1]*100:+.1f},{delta_cam_m[2]*100:+.1f}) "
        f"source={pose_source} depth_status={depth_status}"
    )
    ...
    print(
        f"[REFINE MOVE 5CM POSE] count={correction_count}/{MAX_CORRECTION_COUNT} "
